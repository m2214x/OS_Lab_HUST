...
int s_start(void)
{
  ...
  // init phisical memory manager(only once for the whole system). added @lab2_challenge3
  if(hartid == 0)
    pmm_init();

  spinlock_unlock(&cpu_lock); // unlock the cpu_lock, so that other harts can enter S-mode.
  ...
  sync_barrier(&sync_for_load, NCPU);
