...
ssize_t sys_user_ccwd(uint64 ccva){
  uint64 ccpa = (uint64)user_va_to_pa((pagetable_t)(current->pagetable), (void*)ccva);
  char * ccpa_str = (char *)ccpa;
  return do_ccwd(ccpa);
}
